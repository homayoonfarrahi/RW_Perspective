...
function Plane(pa, pb, pc, pd) {
	this.vh = null; 						// Horizontal vanishing point
	this.vv = null; 						// Vertical vanishing point
	this.cp = null; 						// Central vanishing point
	this.pa = pa; 						// 2D location of perspective point
	this.pb = pb; 						// 2D location of perspective point
	this.pc = pc; 						// 2D location of perspective point
	this.pd = pd; 						// 2D location of perspective point
	this.a = new Point3(0, 0, 0);	               // 3D location of perspective point
	this.b = new Point3(0, 0, 0);	               // 3D location of perspective point
	this.c = new Point3(0, 0, 0);	               // 3D location of perspective point
	this.d = new Point3(0, 0, 0);	               // 3D location of perspective point
	this.tr_ac = null;  				// 2D transverse intersection point with vh-vv horizon line
	this.tr_bd = null;					// 2D transverse intersection point with vh-vv horizon line

  this.computeZforProjectedPlane = function() {
		return this.computeZforProjectedPlaneWithZDepth(1);
	}

  this.computeZforProjectedPlaneWithZDepth = function(zDepth) {
		...
		var avh = 0, bvh = 0, cvh = 0, dvh = 0;
		var avv = 0, bvv = 0, cvv = 0, dvv = 0;
		var az = 0, bz = 0, cz = 0, dz = 0, n = 0;
		...
		this.vh = findIntersect(this.pa, this.pd, this.pb, this.pc);  //horizontal vanishing point of projected plane
		this.vv = findIntersect(this.pa, this.pb, this.pc, this.pd);  //vertical vanishing point of projected plane
		this.cp = findIntersect(this.pa, this.pc, this.pb, this.pd);  //center point of projected plane
		...
		avh = length2D(this.pa, this.vh); avv = length2D(this.pa, this.vv);
		bvh = length2D(this.pb, this.vh); bvv = length2D(this.pb, this.vv);
		cvh = length2D(this.pc, this.vh); cvv = length2D(this.pc, this.vv);
		dvh = length2D(this.pd, this.vh); dvv = length2D(this.pd, this.vv);

		if (avh > 9900000) avh = 9999999;
		if (avv > 9900000) avv = 9999999;
		if (bvh > 9900000) bvh = 9999999;
		if (bvv > 9900000) bvv = 9999999;
		if (cvh > 9900000) cvh = 9999999;
		if (cvv > 9900000) cvv = 9999999;
		if (dvh > 9900000) dvh = 9999999;
		if (dvv > 9900000) dvv = 9999999;
		...
		//determine depth
		az = 1;  //set a.z arbitrarily to 1 (afterwards we'll invert and then multiply to get to correct zDepth)
		bz = az * bvv / avv;  //a to b to vv
		dz = az * dvh / avh;  //a to d to vh
		cz = bz * cvh / bvh;  //b to c to vh (this could also be d to c to vv; cz = dz * cvv / dvv -- same result)
		...
		if (az == n) { }  //a.z is closest to the screen, nothing further to be done

		if (bz == n) {  //b.z is closest to the screen
			bz = 1;  //set to 1
			az = bz * avv / bvv;  //a to b to vv
			dz = az * dvh / avh;  //a to d to vh
			cz = bz * cvh / bvh;  //b to c to vh
		}
		if (cz == n) {  //c.z is closest to the screen
			cz = 1;  //set to 1
			bz = cz * bvh / cvh;  //b to c to vh
			az = bz * avv / bvv;  //a to b to vv
			dz = az * dvh / avh;  //a to d to vh
		}
		if (dz == n) {  //d.z is closest to the screen
			dz = 1;  //set to 1
			cz = dz * cvv / dvv;  //d to c to vv
			az = dz * avh / dvh;  //a to d to vh
			bz = cz * bvh / cvh; //b to c to vh
		}
  ...
  this.getUV = function(u1, u2, u3, v1, v2, v3, px, py) {
		var u = interpolateTrianglePerspective(this.pa.x, this.pa.y, this.a.z, u1,
			this.pb.x, this.pb.y, this.b.z, u2,
			this.pc.x, this.pc.y, this.c.z, u3, px, py);
		var v = interpolateTrianglePerspective(this.pa.x, this.pa.y, this.a.z, v1,
			this.pb.x, this.pb.y, this.b.z, v2,
			this.pc.x, this.pc.y, this.c.z, v3, px, py);
		return new Point2(u, v);
	}

	this.projectTo3D = function(pu, pv) {
		var z = interpolateTriangleLinear(this.pa.x, this.pa.y, this.a.z,
			this.pb.x, this.pb.y, this.b.z,
			this.pc.x, this.pc.y, this.c.z, pu, pv);
		var px = interpolateTriangleLinear(this.pa.x, this.pa.y, this.a.x * this.a.z,
			this.pb.x, this.pb.y, this.b.x * this.b.z,
			this.pc.x, this.pc.y, this.c.x * this.c.z, pu, pv) / z;
		var py = interpolateTriangleLinear(this.pa.x, this.pa.y, this.a.y * this.a.z,
			this.pb.x, this.pb.y, this.b.y * this.b.z,
			this.pc.x, this.pc.y, this.c.y * this.c.z, pu, pv) / z;
		// this is a hack to fix negative z value cases causing wrong projected points
		if (z < 0) {
			return new Point2(10000, 10000);
		}
